...
import { Check, ChevronDown, PencilLine, Plus, SquarePen, Trash2, X } from "lucide-react";
import {
  formatBudgetMoney,
...
export default function CostEstimatorItemModal({
  ...
  const [editableCostIds, setEditableCostIds] = useState<string[]>([]);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[150] bg-black/40 p-4 backdrop-blur-sm">
      ...
      <div className="relative mx-auto flex h-[min(92vh,920px)] w-full max-w-7xl flex-col overflow-hidden rounded-[28px] border border-apple-mist bg-[#f8fbf8] shadow-[0_28px_90px_rgba(15,23,42,0.24)]">
        ...
        <div className="grid h-full min-h-0 flex-1 xl:grid-cols-[320px_minmax(0,1fr)_380px]">
          <section className="flex h-full min-h-0 flex-col border-b border-apple-mist bg-[rgb(var(--apple-snow))] px-6 py-5 xl:border-b-0 xl:border-r">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <button
                ...
                disabled={editingMaterials.length > 0}
          ...
          <section className="flex h-full min-h-0 flex-col border-b border-apple-mist bg-white px-6 py-5 xl:border-b-0 xl:border-r">
            {activeEditingMaterial ? (() => {
              ...
              const isEditable = editableCostIds.includes(materialRow.id);
                      ...
                      <div>
                        <div className="mb-2 flex items-center justify-between gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              setEditableCostIds((current) =>
                                current.includes(materialRow.id)
                                  ? current.filter((id) => id !== materialRow.id)
                                  : [...current, materialRow.id],
                              )
                            }
                            className="inline-flex items-center gap-1 text-xs font-semibold text-[#1f6a37]"
                          >
                            <PencilLine size={12} />
                            {isEditable ? "Use source" : "Edit"}
                          </button>
                        </div>
                        ...
                        {isEditable ? (
                          ...
                          <div className="rounded-[12px] border border-apple-mist bg-[rgb(var(--apple-snow))] px-4 py-3">
                          ...
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              ...
                              <p className="mt-1 text-xs text-apple-steel">
                                {material.unitType || "No unit"} | {material.quantityInput || "0"}
                              </p>
                            </div>
                          ...
                          <div className="mt-2 flex justify-end gap-4">
                            {material.saved ? (
                              <button
                                ...
                                className="inline-flex items-center gap-1 text-xs font-semibold text-apple-charcoal transition hover:text-[#1f6a37]"
                              >
                                <SquarePen size={12} />
                                Edit
                              </button>
