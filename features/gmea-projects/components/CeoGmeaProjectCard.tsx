...
export default function CeoGmeaProjectCard({ project }: { project: GmeaProject }) {
  ...
  return (
    <article className="flex min-w-0 flex-col overflow-hidden rounded-2xl border border-slate-200/70 bg-white shadow-[0_10px_30px_-25px_rgba(15,23,42,.2)]">
      ...
      <Link href={`/gmea-projects/${project.id}`} aria-label={`View details: ${project.name}`} className="flex items-center gap-2 border-t border-slate-100 px-5 py-4 text-xs font-bold text-teal-700 transition hover:bg-teal-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-teal-700">
