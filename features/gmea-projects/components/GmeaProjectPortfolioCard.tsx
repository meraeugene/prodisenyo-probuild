  ...
  return (
    <article style={projectContainerStyle(project)} className="group relative isolate flex min-w-0 flex-col overflow-hidden rounded-[16px] border bg-white shadow-[0_8px_22px_-20px_rgba(15,23,42,.3)] transition-shadow duration-200 hover:shadow-[0_14px_32px_-20px_rgba(15,23,42,.38)]">
      <div className="flex-1 p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="min-w-0">
              <h2 className="truncate text-[16px] font-bold leading-5 tracking-[-0.02em] text-slate-950">{project.title}</h2>
          ...
          <div className="flex items-center gap-2">
            ...
            {canEdit && <GmeaProjectActionsMenu project={project} onDetails={onDetails} />}
          </div>
        </div>
      ...
      <footer className="flex items-center justify-between border-t border-slate-100 bg-slate-50/45 px-5 py-3">
        <Link aria-label={`Open project: ${project.title}`} className="inline-flex items-center gap-2 text-[13px] font-bold text-[#08746f] after:absolute after:inset-0 after:z-10 after:rounded-[16px] after:content-[''] focus-visible:outline-none focus-visible:after:ring-2 focus-visible:after:ring-inset focus-visible:after:ring-teal-700" href={`/gmea-projects/${project.id}`}>
          Open project <ArrowUpRight size={14} aria-hidden="true" />
        </Link>
        {!canEdit && <button type="button" className="relative z-20 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-[12px] font-medium text-slate-600 transition hover:border-slate-300 hover:text-slate-950 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-700" onClick={onDetails}>Details</button>}
      </footer>
    </article>
  );
}
