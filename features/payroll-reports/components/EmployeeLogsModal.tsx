...
import { buildEmployeeAttendanceModalData, formatPayrollReportLogDate, formatPayrollReportLogTime, formatPayrollReportPeriodLabel, formatPayrollReportPeso } from "@/features/payroll-reports/utils/payrollReportHelpers";
import { PayrollReportMetricRow, PayrollReportSummaryCard, PayrollReportSummaryChip } from "@/features/payroll-reports/components/PayrollReportUiBits";
import { formatPayrollNumber } from "@/features/payroll/utils/payrollFormatters";
import type { AttendanceLogRow, PayrollRunItemRow, PayrollRunRow } from "@/features/payroll-reports/types";
          ...
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
            ...
            <div className="overflow-hidden rounded-xl border border-apple-mist bg-white">
              ...
              <div className="grid grid-cols-2 gap-3 p-4">
                ...
                />
                <PayrollReportSummaryChip label="IN Logs" value={modalData.inLogs} />
                <PayrollReportSummaryChip label="OUT Logs" value={modalData.outLogs} />
                <PayrollReportSummaryChip label="OT Logs" value={modalData.otLogs} />
              </div>
            </div>
          </div>
                    ...
                    {modalData.dailyRows.length > 0 ? (
                      modalData.dailyRows.map((row) => (
                        <tr key={`${row.date}|||${row.site}`}>
                          <td className="px-3 py-2 text-apple-smoke">{formatPayrollReportLogDate(row.date)}</td>
                          <td className="px-3 py-2 text-apple-smoke">{row.site}</td>
                          <td className="px-3 py-2 text-apple-smoke">
                            {formatPayrollReportLogTime(row.time1In || null)} - {formatPayrollReportLogTime(row.time1Out || null)}
                          </td>
                          <td className="px-3 py-2 text-apple-smoke">
                            {formatPayrollReportLogTime(row.time2In || null)} - {formatPayrollReportLogTime(row.time2Out || null)}
                          </td>
                          <td className="px-3 py-2 text-apple-smoke">
                            {formatPayrollReportLogTime(row.otIn || null)} - {formatPayrollReportLogTime(row.otOut || null)}
                          </td>
