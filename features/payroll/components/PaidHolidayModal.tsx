...
export default function PaidHolidayModal({
  ...
  useEffect(() => {
    if (!show) return;
    const seedDate = periodStart ? new Date(`${periodStart}T00:00:00`) : new Date();
    ...
    <div className="fixed inset-0 z-[120] bg-black/45 backdrop-blur-sm p-4 flex items-center justify-center">
      <div className="w-full max-w-4xl rounded-2xl border border-apple-mist bg-white shadow-apple-xs p-5 sm:p-6 space-y-4 max-h-[88vh] overflow-y-auto">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h3 className="text-lg font-bold text-apple-charcoal">Paid Holidays</h3>
            <p className="text-sm text-apple-smoke">
              Add holidays automatically (Philippines) or manually using the
              calendar.
            </p>
            {(periodStart || periodEnd) && (
              <p className="mt-1 text-xs font-semibold text-apple-steel">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-4">
          <div className="rounded-2xl border border-apple-mist bg-apple-snow p-4">
          ...
          <div className="rounded-2xl border border-apple-mist bg-white p-4 space-y-3">
            ...
            <button
              ...
              className="w-full h-10 rounded-2xl bg-apple-charcoal text-white text-sm font-semibold hover:bg-apple-black transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Add Holiday
            </button>
              ...
              <div className="space-y-2 max-h-[240px] overflow-y-auto pr-1">
                {holidays.length === 0 ? (
                  ...
                  holidays.map((holiday) => (
                    <div
                      key={holiday.date}
                      className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 flex items-center gap-2"
                    >
                      ...
                      <button
                        ...
                        className="ml-auto px-2.5 py-1 rounded-lg border border-red-200 text-2xs font-semibold text-red-600 hover:bg-red-50 transition"
                      >
                        Remove
                      </button>
                    </div>
